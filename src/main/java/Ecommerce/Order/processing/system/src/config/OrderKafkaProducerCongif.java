...
@Configuration
@EnableKafka
public class OrderKafkaProducerCongif {
    ...
    public ProducerFactory<String, OrderPlacedEvent> producerFactory() {
        ...
        config.put(ProducerConfig.RETRY_BACKOFF_MAX_MS_CONFIG, 1000);

        config.put(ProducerConfig.LINGER_MS_CONFIG, 5);
        config.put(ProducerConfig.BATCH_SIZE_CONFIG, 37878);
    ...
    public KafkaTemplate<String , OrderPlacedEvent> kafkaTemplate(){
        KafkaTemplate<String , OrderPlacedEvent> template = new KafkaTemplate<>(producerFactory());
